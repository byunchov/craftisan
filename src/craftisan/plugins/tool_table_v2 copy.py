...
class DBToolTable(DataPlugin):
    ...
    def recursive_dict_update(self, d1, d2):
        for key, value in d2.items():
            if key in d1 and isinstance(d1[key], dict) and isinstance(value, dict):
                self.recursive_dict_update(d1[key], value)
            else:
                d1[key] = value
    
    def onToolTableFileChanged(self, path):
        LOG.debug('Tool Table file changed: {}'.format(path))
        QTimer.singleShot(50, self.onTTFileChange)

    def onTTFileChange(self):
        if self.tool_table_file not in self.fs_watcher.files():
            self.fs_watcher.addPath(self.tool_table_file)

        tool_table = self.getTTData()
        self.recursive_dict_update(self.tool_table, tool_table)
    ...
    def _loadToolTable(self):
        ...
        filtered_dicts = [
            t for t in self.tool_table.values()
            if any(t.get(key) != file_tt[t['T']].get(key) for key in 'PDZ')
        ]
    ...
    def saveToolTable(self, *args, **kwargs):
        ...
        QTimer.singleShot(20, lambda *args: self.fs_watcher.blockSignals(False))
