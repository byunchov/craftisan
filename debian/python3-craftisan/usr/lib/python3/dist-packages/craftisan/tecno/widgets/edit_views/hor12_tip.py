...
class Horizontal12TipDataWidget(BaseEditWidget):
    ...
    def initUi(self):
        ...
        self.tip1OffsetXLbl = QLabel('Offset X')
        self.tip1OffsetXInput = QDoubleSpinBox()
        self.tip1OffsetXInput.setRange(-9999, 9999)
        self.tip1OffsetXInput.setDecimals(4)

        self.tip1OffsetYLbl = QLabel('Offset Y')
        self.tip1OffsetYInput = QDoubleSpinBox()
        self.tip1OffsetYInput.setRange(-9999, 9999)
        self.tip1OffsetYInput.setDecimals(4)
        ...
        self.tip1DataLayout.addRow(self.tip1OffsetXLbl, self.tip1OffsetXInput)
        self.tip1DataLayout.addRow(self.tip1OffsetYLbl, self.tip1OffsetYInput)
        ...
        self.tip2OffsetXLbl = QLabel('Offset X')
        self.tip2OffsetXInput = QDoubleSpinBox()
        self.tip2OffsetXInput.setRange(-9999, 9999)
        self.tip2OffsetXInput.setDecimals(4)

        self.tip2OffsetYLbl = QLabel('Offset Y')
        self.tip2OffsetYInput = QDoubleSpinBox()
        self.tip2OffsetYInput.setRange(-9999, 9999)
        self.tip2OffsetYInput.setDecimals(4)
        ...
        self.tip2DataLayout.addRow(self.tip2OffsetXLbl, self.tip2OffsetXInput)
        self.tip2DataLayout.addRow(self.tip2OffsetYLbl, self.tip2OffsetYInput)
    ...
    def setSafetyDia(self, value):
        if not self.safetyDataGroup.isChecked():
            safe_dia = self.tip1LAFulcrumInput.value() + value
            self.safetyOTDiaInput.setValue(safe_dia)

    def setSafetyLT(self, value):
        if not self.safetyDataGroup.isChecked():
            safe_len = self.tipOAZInput.value() + value
            self.safetyOTLenInput.setValue(safe_len)
    ...
    def fillToolData(self, tool: Tool):
        ...
        for i in range(2):
            ...
            getattr(self, f"tip{i+1}OffsetXInput").setValue(tool.getTipXOffset(i))
            getattr(self, f"tip{i+1}OffsetYInput").setValue(tool.getTipYOffset(i))
    ...
    def validate(self):
        ...
        xOffsets = tuple(getattr(self, f"tip{i}OffsetXInput").value() for i in range(1, 3))
        yOffsets = tuple(getattr(self, f"tip{i}OffsetYInput").value() for i in range(1, 3))
        ...
        self.tool.setXOffsets(xOffsets)
        self.tool.setYOffsets(yOffsets)
        ...
        self.tool.magdiameter = self.safetyOTDiaInput.value() or max_diameter
        self.tool.maglength = self.safetyOTLenInput.value() or max_length
    ...
    def connectSignals(self):
        ...
        self.tip1LTInput.valueChanged[float].connect(self.setSafetyDia)
        self.tipOFSZInput.valueChanged[float].connect(self.setSafetyLT)